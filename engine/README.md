...
- 4B ([record](../docs/changes/2026-09-25-phase4b-many-lights.md); GPU checks NOT RUN until the laptop runs `run-local.cmd`): `cargo test --release -j 2 -p gpu --test lights -- --test-threads=1 --nocapture` runs G1–G6 (with `NE_GATE_DIR` set, G4's 1080p references are cached there and its display images written for `python results\phase4b\flip.py <NE_GATE_DIR> results\phase4b`); `set NE_NO_VALIDATION=1` and `-- --ignored ... equal_time_curve` runs M1. The viewer's night runs take `--scene night --hour 21`.
