...
- **Many lights (4B, [record](../docs/changes/2026-09-25-phase4b-many-lights.md)):** `ShadeSettings::emitters` adds emitter next-event estimation at the primary vertex and at the bounce hit, by the reference's own function (`shaders/emitter_nee.slang`, shared with `reference.slang`) at the same place in the stream, so frame f equals the reference's sample f with `emitters_direct` and `emitters_indirect` at `max_bounces` 1 (up to 6 rays per pixel); `emitter_samples` k > 1 averages k samples at the primary vertex (the equal-time curve). `Shade::bind_lit` binds the scene's table (the count comes from it); `Shade::bind` binds a placeholder and refuses emitter settings. The shaded radiance is reflected light only: `debug_view::Lighting::emission` adds each surface pixel's emitted radiance in the light view, after reconstruction. `exposure::Exposure` sums the displayed luminance over every 4th pixel into a host-visible slot per frame in flight, for the viewer's automatic exposure. `temporal`: `History::set_lights` (a switch is a light jump), and `Relight` boxes carry the emitter terms E1 (`changed_power`) and E2 (`Relight::with_lights`) of ADR-0006 Amendment 2. Tests: `tests/lights.rs` (G1–G6, and M1 with `--ignored`).
- **Reconstruction (3E):** `denoise::Denoise` (`shaders/denoise.slang`, SVGF-style) demodulates the resolved radiance by albedo, estimates variance from the history moments, runs à-trous levels (default 2) that mix only pixels on the same surface (exact guides) with luminance edge-stopping (the first level on 3×3 means), and remodulates into the shade radiance buffer; `DenoiseTargets` 44 B/px, `GpuTemporal`. Diagnostics: `--ignored sweep_sliders`, `denoise_cost_breakdown`, `motion_images`, `diagnose_motion_edges` (`NE_DIAG_ARM`, `NE_DIAG_HARD_ONLY`, `NE_DIAG_NO_HARD`). Tests: `tests/denoise.rs` (against the estimator converged on a still camera; criteria in the [3E record](../docs/changes/2026-09-24-phase3e-denoise.md)). **Weights (the G4 filter record, [2026-09-26](../docs/changes/2026-09-26-4b-filter-energy.md)):** `DenoiseSettings::weights` is `Svgf` (the default, the 3E / 3G filter), `Symmetric` (ablation) or `Conservative` (symmetric weights and equal-and-opposite exchanges normalised by each pair's larger same-surface kernel sum, packed into the compact key's free bits by init: each surface keeps its energy exactly, no new memory); `DenoiseSettings::parse("conservative:4")`. The tests `lights`, `gate` and `denoise` run their criteria with `NE_FILTER=<weights[:sigma_l]>` (default: the default filter) and prefix their display images with its tag; the FLIP scripts take that prefix as a third argument. `tests/denoise_model.rs` is a device-free CPU mirror of the filter fed with CPU frames of the viewer's transport (`conservative_weights_keep_energy` runs by default; `--ignored model_filter_energy` screens settings, `NE_MODEL_SIZE`, `NE_MODEL_REF_SPP`, `NE_MODEL_DIR`). The viewer's `--filter` and K select the weights.
...
cargo test --release -j 2 -p gpu --test denoise_model -- --nocapture
set NE_MODEL_SIZE=960x540&& set NE_MODEL_REF_SPP=256&& cargo test --release -j 2 -p gpu --test denoise_model -- --ignored --nocapture
set NE_FILTER=conservative:4&& set NE_GATE_DIR=%TEMP%\ne_gate_4b&& cargo test --release -j 2 -p gpu --test lights -- --test-threads=1 --nocapture night_against_the_reference
set NE_FILTER=conservative:4&& set NE_NO_VALIDATION=1&& cargo test --release -j 2 -p gpu --test denoise -- --ignored --test-threads=1 --nocapture filter_cost_against_the_default
target\release\viewer.exe --scene night --hour 17.5 --run-day --filter conservative:4
```

Notes:
- The last five commands come from the [G4 filter record](../docs/changes/2026-09-26-4b-filter-energy.md). They are not verified on the laptop yet: the model ones ran in a cloud session, and the GPU and viewer ones are NOT RUN until `run-local.cmd` returns.
...
- 4B ([record](../docs/changes/2026-09-25-phase4b-many-lights.md); run on the RTX 3050 2026-09-26, G4 and G5 open): `cargo test --release -j 2 -p gpu --test lights -- --test-threads=1 --nocapture` runs G1–G6 (with `NE_GATE_DIR` set, G4's 1080p references are cached there and its display images written for `python results\phase4b\flip.py <NE_GATE_DIR> results\phase4b`); `set NE_NO_VALIDATION=1` and `-- --ignored ... equal_time_curve` runs M1; `-- --ignored ... diagnostic_g4_filter_energy` (validation on, about 40 s with the references cached) measures where the filter changes the night image's energy. The viewer's night runs take `--scene night --hour 21`.
