...
- **When the lights are on** (Phase 4 decision 3; 4A part 2): while the sun is below the horizon (elevation < 0°), `light::emitters::lights_on`. Daytime therefore stays exactly as accepted in M3. At M4's times the lights are off at dusk (17.75 h, +2.65°) and on at blue hour (18.5 h) and night (21 h). The switch is a light jump. The viewer's L and the automatic switch come with 4B.
- **Metric exposure at night** (4A part 2): comparisons on display values (FLIP) take their exposure from the reference image, `light::exposure::metric_exposure`: 0.18 over the log-average luminance of the pixels at least 2⁻¹⁰ of the image mean. The black night sky and noise-level pixels are left out; by day no pixel is that dark, so it is the plain log-average. M3's metric exposure (sun plus sky irradiance) goes to zero at night.
- **Real time (4B):** emission at the primary hit is deterministic: it is added after reconstruction, not demodulated or filtered; the filter's guides stay the surface's own. *Implemented in 4B:* `gpu::shade` samples emitters at the primary vertex and at the bounce hit by the reference's own function (`shaders/emitter_nee.slang`, shared) at the same place in the stream, so frame f equals the reference's sample f with `emitters_direct` and `emitters_indirect` at `max_bounces` 1; the light view adds each surface pixel's emitted radiance (`debug_view::Lighting::emission`). With the lights off no emitter random numbers are drawn. Record: [4B](../changes/2026-09-25-phase4b-many-lights.md).
...
- **Implementation status (2026-09-25):** CPU and GPU references implemented; C1–C5 pass (cloud session) and G1–G5 pass on the RTX 3050, with the M3 GPU regressions unchanged (4A record). Part 2 adds the lights rule and the metric exposure (C7 passes), `ref_light --scene night` for the night references (G8, NOT RUN until the laptop run) and the magnitudes (4A record). The real-time path has no emitters yet (4B).
