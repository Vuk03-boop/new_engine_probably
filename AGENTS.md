# AGENTS.md — single-developer operating contract
...
- This is a **new-engine proposal and starter documentation**, not an implemented engine. No source tree, backend or dependency choice has been approved merely by appearing in the proposal.
- Current authorization is documentation/restoration only. Do not scaffold or implement until the user explicitly authorizes a bounded implementation task. After authorization, work within that scope without asking permission for every ordinary edit.
...
| Area | Rule |
|---|---|
| Current documentation task | May edit current starter docs and add supporting records within this project. No engine implementation. |
| Future authorized implementation | Modify only the agreed source/test/doc surface; include directly required companion changes. Expand scope explicitly if new ownership/layout/behavior boundaries appear. |
...
- Discover runnable commands from the actual project/toolchain. No build commands are validated for this new engine yet; do not copy old commands as if they run here.
