...
- **Implementation status (2026-09-25):** CPU and GPU references implemented; C1–C5 pass (cloud session) and G1–G5 pass on the RTX 3050, with the M3 GPU regressions unchanged (4A record). Part 2 adds the lights rule and the metric exposure (C7 passes), `ref_light --scene night` for the night references (G8, NOT RUN until the laptop run) and the magnitudes (4A record). 4B part 1 (2026-09-26, [record](../changes/2026-09-26-phase4b-many-lights.md)) builds the real-time path's emitters: the lit shade module draws `emitter_spp` samples of this estimator at the primary vertex and at the bounce hit (their mean; with 1 sample the reference's one-bounce stream), and `gpu::compose` adds emission after reconstruction. The lights follow `lights_on` in the viewer, with L by hand. Cloud checks pass; the GPU checks are NOT RUN until the laptop run.
