...
- **The lights in real time (4B part 1, [record](../docs/changes/2026-09-26-phase4b-many-lights.md)):** `shade.slang` is compiled twice. Without `EMITTERS` the module is M3's (byte-identical to main's, criterion C10) and runs whenever the lights are off; `shade_lit` adds the emitter rows (`Shade::bind_lit`) and, with `ShadeSettings::emitters`, `emitter_spp` samples of the reference's emitter estimator at the primary vertex and the bounce hit (up to 4 + 2s rays per pixel). `compose::Compose` (`shaders/compose.slang`) adds emission after the temporal pass and the filter and meters the shown image for the automatic exposure (`ComposeTargets`, host-visible, one buffer per frame slot). `History::set_lights` makes a lights change a full reset (ADR-0006 Amendment 2). Tests: `tests/night.rs` (G10–G14, Q1–Q4, M1–M3; `NE_4B_DIR` caches the 1080p references and converged images and receives the FLIP images for `results/phase4b/flip.py`; `equal_time_curve_cost` is a timing run with `NE_NO_VALIDATION=1`).
...
- 4B part 1 (NOT RUN on a GPU yet; `run-local.cmd` holds the steps): `cargo test --release -j 2 -p gpu --test night -- --test-threads=1 --nocapture <test>` with `NE_4B_DIR` set; `ref_light.exe results\phase4b\ref1 --scene night --bounces 1 --spp 16384 --times blue_hour,night`; `python results\phase4b\flip.py <NE_4B_DIR> results\phase4b results\phase4a_ref results\phase4b\ref1`; the viewer with `--lights`, `--emitter-spp`.
